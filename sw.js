...
}, k = (a) => [d.prefix, a, d.suffix].filter((e) => e && e.length > 0).join("-"), j = (a) => {
...
function D(a) {
  if (!a)
    throw new f("add-to-cache-list-unexpected-type", { entry: a });
  if (typeof a == "string") {
...
async function $(a, e) {
  ...
  }, c = B() ? s.body : await s.blob();
  return new Response(c, n);
}
const G = (a) => new URL(String(a), location.href).href.replace(new RegExp(`^${location.origin}`), "");
function W(a, e) {
  const t = new URL(a);
  for (const s of e)
...
class Y {
  ...
  async cacheMatch(e) {
    const t = R(e);
    let s;
    const { cacheName: r, matchOptions: n } = this._strategy, c = await this.getCacheKey(t, "read"), o = Object.assign(Object.assign({}, n), { cacheName: r });
    s = await caches.match(c, o);
  ...
  async cachePut(e, t) {
    ...
    if (!n)
      return !1;
    const { cacheName: c, matchOptions: o } = this._strategy, i = await self.caches.open(c), l = this.hasCallback("cacheDidUpdate"), u = l ? await V(
...
class Z {
  ...
  handleAll(e) {
    ...
    const t = e.event, s = typeof e.request == "string" ? new Request(e.request) : e.request, r = "params" in e ? e.params : void 0, n = new Y(this, { event: t, request: s, params: r }), c = this._getResponse(n, s, t), o = this._awaitComplete(c, n, s, t);
    return [c, o];
  }
...
class p extends Z {
  ...
  async _handleFetch(e, t) {
    ...
    if (this._fallbackToNetwork) {
      const n = r.integrity, c = e.integrity, o = !c || c === n;
      s = await t.fetch(new Request(e, {
        integrity: e.mode !== "no-cors" ? c || n : void 0
      })), n && o && e.mode !== "no-cors" && (this._useDefaultCacheabilityPluginIfNeeded(), await t.cachePut(e, s.clone()));
    } else
...
class ee {
  ...
  addToCacheList(e) {
    ...
    for (const s of e) {
      ...
      const { cacheKey: r, url: n } = D(s), c = typeof s != "string" && s.revision ? "reload" : "default";
  ...
  install(e) {
    return q(e, async () => {
      ...
      for (const [n, c] of this._urlsToCacheKeys) {
        const o = this._cacheKeysToIntegrities.get(c), i = this._urlsToCacheModes.get(n), l = new Request(n, {
          integrity: o,
          cache: i,
...
let P;
const N = () => (P || (P = new ee()), P);
try {
  self["workbox:routing:7.3.0"] && _();
} catch {
}
const O = "GET", C = (a) => a && typeof a == "object" ? a : { handle: a };
class w {
  /**
...
class se {
  ...
  handleRequest({ request: e, event: t }) {
    ...
    let o = c && c.handler;
    const i = e.method;
    if (!o && this._defaultHandlerMap.has(i) && (o = this._defaultHandlerMap.get(i)), !o)
      return;
    let l;
    try {
      l = o.handle({ url: s, request: e, event: t, params: n });
    } catch (h) {
      l = Promise.reject(h);
    }
    const u = c && c.catchHandler;
    return l instanceof Promise && (this._catchHandler || u) && (l = l.catch(async (h) => {
      if (u)
        try {
          return await u.handle({ url: s, request: e, event: t, params: n });
        } catch (T) {
          T instanceof Error && (h = T);
        }
  ...
  findMatchingRoute({ url: e, sameOrigin: t, request: s, event: r }) {
    ...
    for (const c of n) {
      let o;
      const i = c.match({ url: e, sameOrigin: t, request: s, event: r });
      if (i)
        return o = i, (Array.isArray(o) && o.length === 0 || i.constructor === Object && // eslint-disable-line
        Object.keys(i).length === 0 || typeof i == "boolean") && (o = void 0), { route: c, params: o };
    }
    return {};
  }
  /**
...
let y;
const ae = () => (y || (y = new se(), y.addFetchListener(), y.addCacheListener()), y);
function re(a, e, t) {
  let s;
  if (typeof a == "string") {
    const n = new URL(a, location.href), c = ({ url: o }) => o.href === n.href;
    s = new w(c, e, t);
...
function* ce(a, { ignoreURLParametersMatching: e = [/^utm_/, /^fbclid$/], directoryIndex: t = "index.html", cleanURLs: s = !0, urlManipulation: r } = {}) {
  const n = new URL(a, location.href);
  n.hash = "", yield n.href;
  const c = ne(n, e);
  if (yield c.href, t && c.pathname.endsWith("/")) {
    const o = new URL(c.href);
...
class oe extends w {
  ...
  constructor(e, t) {
    const s = ({ request: r }) => {
      ...
      for (const c of ce(r.url, t)) {
        const o = n.get(c);
        if (o) {
          const i = e.getIntegrityForCacheKey(o);
          return { cacheKey: o, integrity: i };
        }
      }
    };
    super(s, e.strategy);
...
E.onerror = (a) => {
  console.error("[SW] Error opening cache database:", a.target.error);
};
async function I(a) {
  return new Promise((e, t) => {
    const n = L.transaction("requests", "readonly").objectStore("requests").get(a);
    n.onsuccess = () => {
      e(n.result);
    }, n.onerror = () => {
      t(n.error);
...
async function S(a, e) {
  return e instanceof Response ? S(a, {
    body: await e.clone().arrayBuffer(),
    headers: Object.fromEntries(e.headers.entries()),
    timestamp: Date.now()
  }) : new Promise(async (t, s) => {
    const c = L.transaction("requests", "readwrite").objectStore("requests").put({ id: a, ...e, timestamp: Date.now() });
    c.onsuccess = () => {
      t(c.result);
    }, c.onerror = () => {
      s(c.error);
...
async function ue() {
  if (x) return;
  x = !0;
  const a = await I("!cache-rules");
  a && Object.assign(m, a.rules);
}
self.addEventListener("install", () => {
  console.log("[SW] Installing..."), self.skipWaiting();
});
self.addEventListener("activate", (a) => {
  console.log("[SW] Activating..."), a.waitUntil(self.clients.claim());
});
self.addEventListener("message", (a) => {
  var e, t, s, r;
  if (console.log("[SW] Message received:", a.data), ((e = a.data) == null ? void 0 : e.type) === "SET_JWT" && (b = a.data.token, console.log("[SW] JWT updated:", b ? "present" : "empty")), ((t = a.data) == null ? void 0 : t.type) === "SET_BACKEND_URL" && (U = a.data.url, console.log("[SW] Backend URL updated:", U)), ((s = a.data) == null ? void 0 : s.type) === "SET_CACHE_RULE" && (m[a.data.path] = a.data.options, S("!cache-rules", { rules: m }), console.log("[SW] Cache rules updated:", m)), ((r = a.data) == null ? void 0 : r.type) === "CHECK_CACHE") {
    const n = a.data.paths, o = L.transaction("requests", "readonly").objectStore("requests");
    Promise.all(n.map((i) => new Promise((l) => {
      const u = o.get(i);
      u.onsuccess = () => l({ path: i, found: !!u.result }), u.onerror = () => l({ path: i, found: !1 });
    }))).then((i) => {
      const l = {};
      i.forEach((u) => l[u.path] = u.found), a.ports[0] && a.ports[0].postMessage({ type: "CACHE_STATUS", status: l });
    });
  }
});
self.addEventListener("fetch", (a) => {
  ...
  e.pathname.startsWith("/api/") && (console.log("[SW] Intercepted API request:", e.pathname + e.search), b && U && a.respondWith(new Promise(async (t, s) => {
    const r = m[e.pathname];
    let n = null;
    if (r)
      if (n = await I(e.pathname + e.search), !n)
      ...
      else if (r.ttl && Date.now() > n.timestamp + r.ttl)
        console.log("[SW] Cached response expired, attempting to fetch new one.");
      else
        return console.log("[SW] Returning cached response:", n), t(new Response(n.body, { headers: n.headers }));
    const c = new Headers(a.request.headers);
    c.set("Authorization", `Bearer ${b}`), c.set("ngrok-skip-browser-warning", "true");
    const o = new URL(U), i = new URL(a.request.url);
    i.protocol = o.protocol, i.host = o.host, i.port = o.port;
    const l = !["GET", "HEAD"].includes(a.request.method) && a.request.body !== null, u = new Request(i.toString(), {
      method: a.request.method,
      headers: c,
      body: l ? a.request.body : null,
      ...l ? { duplex: "half" } : {},
      mode: "cors",
      credentials: "omit"
    });
    try {
      ...
      h.ok && r && (console.log("[SW] Caching response for:", e.pathname + e.search), S(e.pathname + e.search, h)), t(h);
    } catch (h) {
      console.error("[SW] Fetch failed:", h), n ? (console.log("[SW] Error, returning cached response"), t(new Response(n.body, { headers: n.headers }))) : s(h);
    }
  })));
});
